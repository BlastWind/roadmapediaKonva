...
router.post("/getUserById", (req, res) => {
  ...
  User.findOne({ _id: user_id }).then(user => {
    console.log(user.savedRoadmap);
    let savedRoadmap, draftRoadmap;

    Roadmap.find({ _id: { $in: user.savedRoadmap } }, { thumbnail: 0 })
      .then(roadmaps => {
        console.log(roadmaps);
        savedRoadmap = roadmaps;
        //  res.json({ userInfo: user, savedRoadmapsInfo: roadmaps });
      })
      .then(
        Roadmap.find({ _id: { $in: user.draftRoadmap } }, { thumbnail: 0 })
          .then(roadmaps => {
          ...
          .then(() => {
            ...
            //console.log("saved", savedRoadmap, "draft", draftRoadmap);
            res.json({
              userInfo: user,
